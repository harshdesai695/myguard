...
class _NoticeManagementScreenState extends State<NoticeManagementScreen> {
  ...
  Future<void> _load() async {
    ...
    // GET /api/v1/communications/notices
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(onPressed: () => context.push('/admin/notices/create'), icon: const Icon(Icons.add_rounded), label: Text('Create Notice'), backgroundColor: AppColors.primary, foregroundColor: AppColors.onPrimary),
      body: _loading ? const AppShimmerList() : RefreshIndicator(onRefresh: _load, child: ListView(children: [
        const SizedBox(height: 200),
        AppEmptyWidget(message: 'No data yet. Pull to refresh.', icon: Icons.campaign_outlined, actionLabel: 'Refresh', onAction: _load),
      ])),
    );
  }
}
